...
class Book {
  constructor(bookDomElement, onPageVisible, onPageInvisible) {
    ...
    this._onPageVisible = onPageVisible || function() {};
    this._onPageInisible = onPageInvisible || function() {};
  ...
  init(bookDomElement) {
    ...
    for (let i = 0; i < this._pages.length; i++) {
      this._pageVisibilities[i] = false;
    }
    //this._pageVisibilities = new Array(this._pages.length).map(() => false);
  ...
  _setPageVisibility(pageIndex, visible) {
    if (this._pageVisibilities[pageIndex] === undefined || this._pageVisibilities[pageIndex] === visible) {
      return;
    }
    this._pageVisibilities[pageIndex] = visible;
    if (visible) {
      this._pages[pageIndex].classList.add('book__page_visible');
      this._onPageVisible(this._pages[pageIndex], pageIndex);
    } else {
      this._pages[pageIndex].classList.remove('book__page_visible');
      this._onPageInisible(this._pages[pageIndex], pageIndex);
    }
  }
  ...
  _dragStart(e) {
    ...
    this._mouseStartTime = new Date().getTime();
  }
  ...
  _dragEnd(e, doNotTurnPage) {
    ...
    if (!doNotTurnPage) {
      if (new Date().getTime() - this._mouseStartTime < this.SHORT_CLICK_TIME) {
        this._mouseStart = null;
        this._mouseStartTime = null;
        this._startFinishing(true);
        return;
      }
    }
  ...
  _startFinishing(reverse) {
    this._stopFinishing();

    this._finishingDirection = (Math.sign(this._activePagePosition) || -1) * (reverse ? -1 : 1);
